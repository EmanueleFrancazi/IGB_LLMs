...
| `test_predictive_measurement.py` | Rank-before-average in the measurement path, argmax identity with greedy, target probability and loss, and that enabling the diagnostics changes no existing count or RNG draw with the sweep on or off | Unit tests | `evaluation/init_distribution.py` |
| `test_predictive_probabilities.py` | Ranked-profile invariants, sufficient-statistic persistence, backward compatibility, figures 8 and 9 | Unit and filesystem tests | `analysis/predictive.py`, `analysis/records.py`, `analysis/figures.py` |
| `test_temperature_confidence.py` | Exact greedy-argmax invariance across temperature, sharpening and flattening limits on analytic logits, canonical-slice agreement, figures 11-13 | Unit and filesystem tests | `analysis/predictive.py`, `analysis/records.py`, `analysis/figures.py` |
| `test_temperature_gradients.py` | Temperature inside the loss: analytic low-T limits for correct and incorrect greedy predictions, invariance of `q_i`/`n_i`/`p_i`, canonical-row identity, six-panel figure 10 | Unit and filesystem tests | `evaluation/position_gradients.py`, `analysis/gradients.py`, `analysis/figures.py` |
| `test_initialization_scale.py` | Paired scale intervention: literal no-op at alpha=1, exact alpha-times-baseline pairing, deterministic gains and buffers untouched, absence of biases, tied-parameter guard | Unit tests | `models/initialization_scale.py` |
| `test_mean_token_probabilities.py` | Average-by-identity-then-rank versus rank-then-average, per-initialization ranking, canonical-row generalization of `mean_predicted_probabilities`, top-k delta, figure 14 | Unit and filesystem tests | `analysis/predictive.py`, `analysis/figures.py` |
| `test_scale_comparison.py` | Cross-alpha greedy diagnostics: position-wise agreement kept distinct from distributional TV, support and zero-frequency deltas, graceful absence of per-position identities | Unit tests | `analysis/scale_comparison.py` |
