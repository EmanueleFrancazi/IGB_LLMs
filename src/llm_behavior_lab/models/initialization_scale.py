...
def _is_deterministic(name: str) -> bool:
    return any(name.endswith(suffix) for suffix in DETERMINISTIC_PARAMETER_SUFFIXES)
...
def classify_parameters(model: nn.Module) -> dict[str, bool]:
    ...
    Classification is by name against the audited deterministic set, so a tensor
    added later is scaled by default and has to be excluded deliberately. That
    is the safer direction to fail: a new random tensor silently left unscaled
    would break the pairing without any symptom.
    """
    ...
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        classified[name] = not _is_deterministic(name)
    return classified
